...
def find_possible_values(grid: tp.List[tp.List[str]], pos: tp.Tuple[int, int]) -> tp.Set[str]:
    ...
    True
    """
    # Все возможные числа для Судоку
    possible_values = set("123456789")

    # Удаляем числа, которые уже есть в строке, столбце или блоке
...
def solve(grid: tp.List[tp.List[str]]) -> tp.Optional[tp.List[tp.List[str]]]:
    ...
    if not empty_pos:
        # Если свободных позиций нет, пазл решён
        return grid

    row, col = empty_pos

    # Найти все возможные значения для данной позиции
    possible_values = find_possible_values(grid, (row, col))

    for value in possible_values:
        # Поместить значение на текущую позицию
        grid[row][col] = value

        # Рекурсивно попробовать решить оставшийся пазл
        if solve(grid):
            return grid
